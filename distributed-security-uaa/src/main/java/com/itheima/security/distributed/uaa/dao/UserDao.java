...
public class UserDao {
    ...
    public UserDto getUserByUsername(String username){
        String sql = "select id,username,password,fullname,mobile from sys_user where username = ?";
        //连接数据库查询用户
    ...
    public List<String> findPermissionsByUserId(String userId){
        String sql = "SELECT * FROM sys_permission WHERE id IN(\n" +
                ...
                "  SELECT RID FROM sys_user_role WHERE UID = ? \n" +
                ")\n" +
                ")\n";

        List<PermissionDto> list = jdbcTemplate.query(sql, new Object[]{userId}, new BeanPropertyRowMapper<>(PermissionDto.class));
        List<String> permissions = new ArrayList<>();
        list.forEach(c -> permissions.add(c.getPermission_NAME()));
        return permissions;
    }
}
